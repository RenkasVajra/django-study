...
def demand():
    ...
    df1 = average_salary_all.rename({'average_sal': 'Средняя зарплата'}).reset_index().to_csv(index=False)  # зп для всех
    df2 = years_df.set_index('Год').rename(
        columns={'vacancy_count': 'Количество вакансий'}).reset_index().to_csv(index=False)   # кол-во вакансии
